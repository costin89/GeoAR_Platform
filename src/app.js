...
import { loadModelTestAnchors } from "./debug/modelTest.js";
...
const modelTestToggle = document.getElementById("model-test-toggle");
...
// Phase 7: Modell-Test lädt asynchron (fetch), Anchor-Liste bleibt bis
// zum ersten erfolgreichen Laden leer.
let modelTestEnabled = false;
let modelTestAnchors = [];

function updateSceneAnchors() {
  const roseAnchors = compassRoseEnabled ? createCompassRoseAnchors() : [];
  const modelAnchors = modelTestEnabled ? modelTestAnchors : [];
  setSceneAnchors(scene, [...placesAnchors, ...roseAnchors, ...modelAnchors]);
}
...
modelTestToggle.addEventListener("click", async () => {
  modelTestEnabled = !modelTestEnabled;
  modelTestToggle.textContent = `Modell-Test: ${modelTestEnabled ? "an" : "aus"}`;
  if (modelTestEnabled && modelTestAnchors.length === 0) {
    try {
      modelTestAnchors = await loadModelTestAnchors();
    } catch (error) {
      overlay.setRow("modelTest", "Modell-Test", `Fehler: ${error.message}`);
      modelTestEnabled = false;
      modelTestToggle.textContent = "Modell-Test: aus";
      return;
    }
  }
  updateSceneAnchors();
