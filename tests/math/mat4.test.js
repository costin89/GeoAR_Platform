...
test("mat4.fromRotationTranslationScale: reine Skalierung (Identitäts-Rotation)", () => {
  const m = mat4.fromRotationTranslationScale(mat4.create(), [0, 0, 0, 1], [0, 0, 0], [2, 3, 4]);
  const p = [1, 1, 1];
  const x = m[0] * p[0] + m[4] * p[1] + m[8] * p[2] + m[12];
  const y = m[1] * p[0] + m[5] * p[1] + m[9] * p[2] + m[13];
  const z = m[2] * p[0] + m[6] * p[1] + m[10] * p[2] + m[14];
  assertClose(x, 2, 1e-9);
  assertClose(y, 3, 1e-9);
  assertClose(z, 4, 1e-9);
});

test("mat4.fromRotationTranslationScale: Skalierung + Translation kombiniert", () => {
  const m = mat4.fromRotationTranslationScale(mat4.create(), [0, 0, 0, 1], [10, 0, 0], [1, 1, 1]);
  const p = [0, 0, 0];
  const x = m[0] * p[0] + m[4] * p[1] + m[8] * p[2] + m[12];
  assertClose(x, 10, 1e-9);
});
